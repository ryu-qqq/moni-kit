...
public class TestLogEntryProvider {
    ...
    public static ExecutionLog executionTimeLog() {
        return ExecutionLog.of(TRACE_ID, CLASS_NAME, METHOD_NAME, EXECUTION_TIME);
    }

    public static DatabaseQueryLog databaseQueryLog() {
        return DatabaseQueryLog.of(TRACE_ID, QUERY, EXECUTION_TIME, DATA_SOURCE, TABLE_NAME, ROWS_AFFECTED, RESULT_SIZE, LOG_LEVEL);
    }

    public static ExceptionLog exceptionLog() {
        return ExceptionLog.of(TRACE_ID, new RuntimeException("Test Exception"));
    }

    public static HttpInboundRequestLog httpInboundRequestLog() {
        return HttpInboundRequestLog.of(TRACE_ID, LOG_LEVEL, URI, HTTP_METHOD, QUERY_PARAMS, REQUEST_BODY, HEADERS, CLIENT_IP, USER_AGENT);
    }

    public static HttpInboundResponseLog httpInboundResponseLog() {
        return HttpInboundResponseLog.of(TRACE_ID, LOG_LEVEL, HTTP_METHOD, TARGET_URL, STATUS_CODE, HEADERS, RESPONSE_BODY, EXECUTION_TIME);
    }

    public static BatchJobLog batchJobLog() {
        return BatchJobLog.of(TRACE_ID, BATCH_JOB_NAME, START_TIME, END_TIME, EXECUTION_TIME, STATUS, "SUCCESS", ERROR_MESSAGE, LOG_LEVEL);
    }

    public static HttpOutboundRequestLog httpOutboundRequestLog() {
        return HttpOutboundRequestLog.of(TRACE_ID, LOG_LEVEL, TARGET_URL, HTTP_METHOD, HEADERS, QUERY_PARAMS, REQUEST_BODY);
    }

    public static HttpOutboundResponseLog httpOutboundResponseLog() {
        return HttpOutboundResponseLog.of(TRACE_ID, LOG_LEVEL, HTTP_METHOD, TARGET_URL, STATUS_CODE, HEADERS, RESPONSE_BODY, EXECUTION_TIME);
    }

    public static ExecutionDetailLog executionDetailLog() {
        return ExecutionDetailLog.of(TRACE_ID, CLASS_NAME, METHOD_NAME, EXECUTION_TIME, INPUT_PARAMS, OUTPUT_VALUE, 1000, "TEST");
    }

...
장애를 빠르게 감지하고 대응하기 위해 로깅 시스템을 구축하면서, **로깅 작업**이 예상보다 훨씬 커진다는 사실을 깨달았습니다. 

특히 서버가 늘어날 때마다 로그 포맷을 일일이 맞춰야 하니, 기존 서버의 로그 포맷을 다시 확인하는 비효율적인 과정이 필요했고, 

새로운 팀원이 들어올 때마다 이 로그 포맷을 매번 설명해줘야 하는건 뻔한 일 입니다. 게다가 이 로그 포맷을 복사하고 붙여넣으면서 개발을 하다 보면 휴먼 에러가 발생할 확률이 높아졌습니다.
